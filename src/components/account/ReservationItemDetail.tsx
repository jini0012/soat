...
function DetailDataLi({ label, data }: DetailDataProps) {
  return (
    <li className="flex gap-2 mb-1">
      ...
      <span className="flex-[3]">{data}</span>
    </li>
  );
}
    ...
    <>
      <section className="relative border-b-2">
        ...
        <Button
          highlight
          href="/demo/ticket"
          className="absolute top-0 right-0 text-xs font-normal py-[2.5px] py-4"
        >
          티켓 확인
        </Button>
        <div className="flex items-center gap-6 my-4">
          <Image
            className={`bg-flesh-500 rounded-[10px] mb-1 w-[90px] h-[130px]`}
            src={`/imageUrl`}
            alt="공연 info"
            width={90}
            height={130}
      ...
      <section>
        ...
        <p className="text-[10px] text-flesh-500">
